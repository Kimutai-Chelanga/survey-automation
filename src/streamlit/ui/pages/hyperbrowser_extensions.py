"""
Fixed Hyperbrowser Extension Management
...
PostgreSQL account linking similar to how profiles are linked.

Key fixes:
1. Proper extension upload using file path (not SDK models)
2. Correct session creation with extensionIds parameter
3. Better error handling and validation
4. Extension validation and management
5. FIXED: Proper extension-to-account linking like profiles
"""
...
"""
Fixed Extension Linking Functions for Account Creation

This fixes the extension linking issue that's causing account creation to fail.
The main problems are:
1. Missing proper error handling in link_extension_to_postgres_account
2. Circular import issues with hyperbrowser_extensions
3. Verification failures due to timing issues

Key fixes:
1. Better error handling and retry logic
2. Proper import structure
3. Immediate verification after linking
4. Fallback strategies when linking fails
"""
...
def create_session_with_extension(profile_id, extension_id=None, session_config=None, **context):
    """
    Create session - extension_id=None means NO extension
    """
    try:
        ...
        # Get URLs to open
        urls_to_open = config.get('urls_to_open', ['https://twitter.com'])
        start_url = urls_to_open[0]
        
        logging.info(f"Creating session for profile: {profile_id}")
        logging.info(f"Extension: {'NONE' if extension_id is None else extension_id}")
        logging.info(f"URLs to open: {urls_to_open}")
        ...
        if extension_id:
            ...
            logging.info("Session will NOT include any extensions")
        ...
        try:
            ...
            try:
                ...
                store_in_mongodb = hyperbrowser_utils.store_in_mongodb
        ...
        return {
            ...
            'extension_loaded': bool(extension_id),
            'urls_opened': urls_to_open,
            'message': f"Session created {'WITH' if extension_id else 'WITHOUT'} extension"
        }
...
def verify_extension_account_link(extension_id, postgres_account_id, max_retries=3, **context):
    """
    FIXED: Verify extension-account link with retries
    """
    import time
        ...
        try:
            ...
            if extension:
                return {
                    ...
                    'extension_id': extension_id,
                    'postgres_account_id': postgres_account_id,
                    'account_username': extension.get('account_username'),
                    'verified_at': datetime.now()
                }
            else:
                if attempt < max_retries - 1:
                    logging.warning(f"Verification attempt {attempt + 1} failed, retrying...")
                    time.sleep(1)
                    continue
                
                return {
                    'linked': False,
                    'error': f'Extension {extension_id} not properly linked to account {postgres_account_id} after {max_retries} attempts'
                }
            ...
            if attempt < max_retries - 1:
                logging.warning(f"Verification error attempt {attempt + 1}: {e}")
                time.sleep(1)
                continue
...
def get_or_create_extension_for_account(postgres_account_id, username, extension_name="automa-extension", force_new=False, **context):
    """
    FIXED: Get or create extension for account with better error handling and fallback strategies
    """
    try:
        logging.info(f"Getting or creating extension for account {postgres_account_id} ({username})")
        ...
        # Force new extension if requested
        if not force_new:
            # Check if this account already has an extension linked
            existing_account_extension = None
            try:
                client, db = get_mongodb_client()
                existing_account_extension = db.extension_instances.find_one({
                    'postgres_account_id': postgres_account_id,
                ...
                logging.warning(f"Could not check for existing account extension: {e}")
            
            if existing_account_extension:
                extension_id = existing_account_extension['extension_id']
                logging.info(f"Found existing extension for account {postgres_account_id}: {extension_id}")
                return {
                    'extension_id': extension_id,
                    'extension_exists': True,
                    'action': 'used_existing_account_extension',
                    'mongodb_id': str(existing_account_extension['_id']),
        ...
        # Check for general unlinked extension that we can link
        if not force_new:
            existing_general_extension = None
            try:
                client, db = get_mongodb_client()
                existing_general_extension = db.extension_instances.find_one({
                    'extension_name': extension_name,
                    'is_enabled': True,
                    'installation_status': 'active',
                ...
                logging.warning(f"Could not check for general extension: {e}")
            
            if existing_general_extension:
                extension_id = existing_general_extension['extension_id']
                logging.info(f"Found unlinked extension, attempting to link to account {postgres_account_id}: {extension_id}")
                
                # Try to link it to this account
                ...
                if link_success:
                    return {
                        'extension_id': extension_id,
                        'extension_exists': True,
                        'action': 'linked_existing_to_account',
                        'mongodb_id': str(existing_general_extension['_id']),
                        'linked_to_account': True,
                        'postgres_account_id': postgres_account_id,
                        'account_username': username,
                        'verified': True
                    ...
                    logging.warning(f"Failed to link extension {extension_id} to account {postgres_account_id}, will create new one")
        
        # Create new extension and link it
        logging.info(f"Creating new extension for account {postgres_account_id}")
        
        # Validate extension file first
        extension_path = os.environ.get('AUTOMA_EXTENSION_PATH', '/app/automa-extension/automa-extension.zip')
        if not os.path.exists(extension_path):
            raise FileNotFoundError(f"Extension file not found: {extension_path}")
        
        # Create the extension
        upload_result = upload_automa_extension_for_account(
            extension_name=extension_name, 
        ...
        if not upload_result.get('extension_id'):
            raise Exception("Extension upload failed - no extension_id returned")
        
        extension_id = upload_result['extension_id']
        logging.info(f"New extension uploaded: {extension_id}")
        
        # The extension should already be linked during upload, but verify
        verification = verify_extension_account_link(extension_id, postgres_account_id, **context)
        if not verification['linked']:
            raise Exception(f"Extension link verification failed: {verification.get('error')}")
        
        return {
            'extension_id': extension_id,
            'extension_exists': False,
            'action': 'created_new_and_linked_to_account',
            'mongodb_id': upload_result['mongodb_id'],
            'linked_to_account': True,
            'postgres_account_id': postgres_account_id,
            'account_username': username,
        ...
        error_msg = f"Failed to get or create extension for account {postgres_account_id}: {str(e)}"
        logging.error(error_msg)
        
        # FALLBACK STRATEGY: Try to proceed without extension
        logging.warning("Attempting fallback strategy: creating account without extension")
        
        try:
            # Check if we can find ANY working extension to use as fallback
            client, db = get_mongodb_client()
            fallback_extension = db.extension_instances.find_one({
                'extension_name': extension_name,
                'is_enabled': True,
                'installation_status': 'active'
            })
            ...
            if fallback_extension:
                extension_id = fallback_extension['extension_id']
                logging.info(f"Using fallback extension (unlinked): {extension_id}")
                
                return {
                    'extension_id': extension_id,
                    'extension_exists': True,
                    'action': 'fallback_unlinked_extension',
                    'mongodb_id': str(fallback_extension['_id']),
                    'linked_to_account': False,  # Not linked but usable
                    'postgres_account_id': postgres_account_id,
                    'account_username': username,
                    'verified': False,
                    'warning': 'Using unlinked extension as fallback'
                }
            
        except Exception as fallback_error:
            logging.error(f"Fallback strategy also failed: {fallback_error}")
        
        # If all else fails, raise the original error
        raise Exception(error_msg)


def upload_automa_extension_for_account(extension_name="automa-extension", extension_path=None, postgres_account_id=None, username=None, **context):
    """
    FIXED: Upload extension and immediately link to account
    """
    try:
        if not HYPERBROWSER_API_KEY:
            raise ValueError("HYPERBROWSER_API_KEY is required")
        ...
        # FIXED: Use correct upload method
        try:
            from hyperbrowser.models.extension import CreateExtensionParams
            extension_response = hb_client.extensions.create(
                CreateExtensionParams(
                    name=extension_name,
                    file_path=extension_path
                )
            )
        except Exception as model_error:
            # Fallback to direct method
            logging.warning(f"Model-based upload failed, trying direct: {model_error}")
            extension_response = hb_client.extensions.create(
                name=extension_name,
                file_path=extension_path
            )
        ...
        # Store in MongoDB with account linking
        extension_data = {
            'extension_id': extension_id,
            'extension_name': extension_name,
            ...
            'installation_status': 'active',
            'is_enabled': True,
            'installation_path': extension_path,
            ...
            # Link to account immediately if provided
            'postgres_account_id': postgres_account_id,
            'account_username': username,
            'linked_to_postgres': bool(postgres_account_id),
            'postgres_link_created_at': datetime.now() if postgres_account_id else None,
            
            'extension_metadata': {
                'permissions': ['activeTab', 'storage', 'scripting'],
                'content_scripts': True,
                'background_service_worker': True,
                'uploaded_via': 'fixed_account_specific_upload',
                'hyperbrowser_response': {
                    'id': extension_id,
                    'upload_timestamp': datetime.now().isoformat()
                },
                'created_for_account': postgres_account_id,
        ...
        try:
            ...
            try:
                from streamlit_hyperbrowser_manager import store_in_mongodb
            except ImportError:
                import streamlit_hyperbrowser_manager
                store_in_mongodb = streamlit_hyperbrowser_manager.store_in_mongodb
        ...
        return {
            ...
            'action': 'uploaded_new_with_account_link',
            'mongodb_id': mongodb_id,
            'postgres_account_id': postgres_account_id,
            'account_username': username,
            'immediately_linked': bool(postgres_account_id)
        }
        
    except Exception as e:
        logging.error(f"Error uploading extension for account {postgres_account_id}: {str(e)}")
        raise
    
def upload_automa_extension(**context):
    """
    FIXED: Upload Automa extension using the correct API approach
    """
    try:
        from hyperbrowser.models.extension import CreateExtensionParams
        if not HYPERBROWSER_API_KEY:
        ...
        if existing_extension:
            ...
            logging.info(f"Using existing extension: {extension_id}")
            return {
                'extension_id': extension_id,
        ...
        # FIXED: Use correct parameters - only file_path
        extension_response =  hb_client.extensions.create(
            CreateExtensionParams(
...
def get_or_create_extension(extension_name="automa-extension", extension_path=None, **context):
    """
    Get existing extension or create a new one if needed
    
    Args:
        extension_name (str): Name of the extension
        extension_path (str): Path to extension ZIP file
        **context: Additional context
    
    Returns:
        dict: Extension information
    """
    try:
        ...
        if existing_extension:
            extension_id = existing_extension['extension_id']
            logging.info(f"Found existing extension: {extension_id}")
        ...
        # No existing extension, create new one
        logging.info(f"No existing extension found, uploading new one: {extension_name}")
        return upload_automa_extension(extension_name=extension_name, extension_path=extension_path, **context)
        
    except Exception as e:
        error_msg = f"Failed to get or create extension: {str(e)}"
        logging.error(error_msg)
        raise Exception(error_msg)
"""
...
"""
def setup_complete_session_with_extension(profile_id=None, extension_name="automa-extension", session_config=None, **context):
    """
    Complete setup: ensure extension exists and create session with it
    
    Args:
        profile_id (str): Profile ID to use
        extension_name (str): Extension name to use
    ...
    try:
        ...
        logging.info(f"Starting complete session setup with profile: {profile_id}")
        
        # Step 1: Ensure extension exists
        logging.info("Step 1: Getting or creating extension...")
        extension_result = get_or_create_extension(extension_name=extension_name, **context)
        extension_id = extension_result['extension_id']
        
        # Step 2: Create session with extension
        logging.info(f"Step 2: Creating session with extension {extension_id}...")
        ...
        complete_result = {
            ...
            'setup_complete': True,
            'extension_setup': extension_result,
            'session_setup': session_result,
            'mongodb_session_id': session_result['mongodb_id'],
            'message': f"Complete setup successful - Session: {session_result['session_id']}, Extension: {extension_id}"
        }
        
        logging.info("Complete session setup successful!")
        return complete_result
        
    except Exception as e:
        error_msg = f"Failed to setup complete session with extension: {str(e)}"
        logging.error(error_msg)
        raise Exception(error_msg)

def list_available_extensions(**context):
    """List all available extensions from MongoDB"""
    try:
        client, db = get_mongodb_client()
...
def deactivate_extension(extension_id, reason='manual_deactivation', **context):
    """Deactivate an extension"""
    try:
        update_result = update_in_mongodb('extension_instances',
            {'extension_id': extension_id},
            {
                'is_enabled': False,
        ...
        logging.info(f"Deactivated extension {extension_id}")
        return update_result
        
    except Exception as e:
        logging.error(f"Failed to deactivate extension {extension_id}: {e}")
        return False
...
def test_extension_workflow(profile_id, **context):
    """Test the complete extension workflow"""
    try:
        logging.info("=== Testing Extension Workflow ===")
        
        # Test 1: Validate extension file
        logging.info("Test 1: Validating extension file...")
        validate_extension_file(AUTOMA_EXTENSION_PATH)
        logging.info("✅ Extension file validation passed")
        
        # Test 2: Upload or get extension
        logging.info("Test 2: Getting or creating extension...")
        extension_result = get_or_create_extension()
        logging.info(f"✅ Extension ready: {extension_result['extension_id']}")
        
        # Test 3: Create session with extension
        logging.info("Test 3: Creating session with extension...")
        session_result = create_session_with_extension(
            profile_id=profile_id,
            extension_id=extension_result['extension_id'],
            session_config={
                'session_purpose': 'testing',
                'start_url': 'https://twitter.com'
            }
        )
        logging.info(f"✅ Session created: {session_result['session_id']}")
        
        # Test 4: Verify session has extension
        browser_url = session_result['browser_url']
        logging.info(f"✅ Browser URL: {browser_url}")
        logging.info(f"✅ Extension loaded: {session_result['extension_loaded']}")
        
        logging.info("=== Extension Workflow Test Complete ===")
        ...
        logging.error(f"Extension workflow test failed: {e}")
        return {
            'test_passed': False,
            'error': str(e)
        }