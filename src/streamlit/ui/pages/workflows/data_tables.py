...
def render_workflow_tables(workflow_type: str, filters: Dict[str, Any]):
    """
    Renders the full workflow table and inspector.
    Passes the appropriate db_module based on workflow_type.
    """
    ...
    # Map workflow type to the correct database module
    db_module_map = {
        "Replies": replies_workflows,
        "Messages": messages_workflows,
        "Retweets": retweets_workflows,
    }

    # Get the appropriate db_module
    db_module = db_module_map.get(workflow_type)

    if db_module is None:
        st.error(f"Unknown workflow type: {workflow_type}")
        return

    # Create renderer with both required arguments
    renderer = WorkflowContentRenderer(workflow_type, db_module)
    renderer.render()
