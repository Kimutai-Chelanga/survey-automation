...
def display_batch_details(batch_id: str, batches_state: dict, screenshot_labels: dict = None):
    ...
    with tab_logs:
        ...
        if logs:
            ...
            st.download_button("⬇️ Download logs", "\n".join(logs), f"logs_{batch_id}.txt", key=f"dl_log_{batch_id}_ui")
        else:
            ...
            for i, (num, img_path, label) in enumerate(shots):
                ...
                if os.path.exists(img_path):
                    ...
                    with open(img_path, "rb") as f:
                        img_bytes = f.read()
                    st.download_button(f"⬇️ {display_label}.png", img_bytes, f"ss_{batch_id}_{i}_{label}.png", mime="image/png", key=f"dl_ss_{batch_id}_ui_{i}")
                else:
...
def display_screening_results_tab(acct: dict, site: dict, batches_state: dict):
    ...
    if st.session_state.get("selected_batch_for_details"):
        ...
        display_batch_details(bid, batches_state)
    ...
    if st.button("📥 Export CSV", key="exp_csv_ui"):
        ...
        st.download_button("⬇️ Download CSV", buf.getvalue(), f"screening_{acct['username']}_{site['site_name'].replace(' ','_')}.csv", mime="text/csv", key="dl_csv_ui")
