...
def get_filtered_workflows_from_mongodb(filters: Dict[str, Any] = None, limit: int = None) -> List[Dict[str, Any]]:
    ...
    try:
        query = filters or {}
        # Filter for retweets content type
        query["content_type"] = "retweets"
        ...
        logger.info(f"Retrieved {len(workflows)} retweet workflow links from MongoDB.")
        return workflows
    except Exception as e:
        logger.error(f"Error fetching retweet workflow links from MongoDB: {e}")
        if STREAMLIT_AVAILABLE:
            st.error(f"❌ Error fetching retweet workflow links from MongoDB: {str(e)}")
        return []
...
def create_workflow_with_content_link(
    ...
    try:
        ...
        logger.info(f"✅ Created retweet workflow link in MongoDB with ID: {result.inserted_id}")
        logger.info(f"   Linked to retweet_id: {content_id}, account_id: {account_id}")
        
        # Create initial execution record
        _create_initial_execution_record(
            automa_workflow_id, result.inserted_id, content_id, account_id, "retweets"
        )
        
        return str(result.inserted_id)
    except Exception as e:
        logger.error(f"❌ Error creating retweet workflow link in MongoDB: {e}")
        if STREAMLIT_AVAILABLE:
            st.error(f"❌ Error creating retweet workflow link in MongoDB: {str(e)}")
        return None
