# hyperbrowser_profile_manager.py
"""
...
DAGs that need to interact with Hyperbrowser profiles.
...
def check_existing_profile(profile_type='hyperbrowser_automa', **context):
    """
    ...
    """
    try:
        # Check for existing profile in MongoDB
        existing_profile = get_from_mongodb('chrome_profiles', {
            'is_active': True,
        ...
        if existing_profile:
            ...
            logging.info(f"Found existing profile ID in MongoDB: {profile_id}")
            ...
            try:
                ...
                # Store results in XCom if context available
                if context and 'task_instance' in context:
                    context['task_instance'].xcom_push(key='existing_profile_id', value=profile_id)
                    ...
                    context['task_instance'].xcom_push(key='profile_mongodb_id', value=str(existing_profile['_id']))
                ...
                update_in_mongodb('chrome_profiles', 
...
def create_hyperbrowser_profile(profile_type='hyperbrowser_automa', profile_name=None, skip_if_exists=True, **context):
    """
    Create a new Hyperbrowser profile and store in MongoDB
    ...
    if skip_if_exists:
        if context and 'task_instance' in context:
            ...
            existing_mongodb_id = context['task_instance'].xcom_pull(
                task_ids='check_existing_profile', 
                key='profile_mongodb_id'
            )
        else:
            # Check directly if no XCom available
            check_result = check_existing_profile(profile_type=profile_type, **context)
    ...
    if skip_creation and existing_profile_id:
        ...
        if context and 'task_instance' in context:
            ...
            context['task_instance'].xcom_push(key='profile_mongodb_id', value=existing_mongodb_id)
    ...
    if not HYPERBROWSER_API_KEY:
        raise ValueError("HYPERBROWSER_API_KEY is required to create profiles")
    ...
    try:
        ...
        # Store in chrome_profiles collection
        mongodb_id = store_in_mongodb('chrome_profiles', profile_data, **context)
        
        # Store in XCom if context available
        if context and 'task_instance' in context:
            ...
            context['task_instance'].xcom_push(key='profile_mongodb_id', value=mongodb_id)
...
def setup_profile_with_automa(profile_id=None, extension_id=None, session_purpose='setup', **context):
    """
    Setup the profile with Automa extension and store session in MongoDB
    
    Args:
        profile_id (str): Profile ID to use (optional, will get from XCom if not provided)
        extension_id (str): Extension ID to use (optional, will get from XCom if not provided)
    ...
    # Get profile_id from XCom if not provided
    if not profile_id and context and 'task_instance' in context:
        profile_id = context['task_instance'].xcom_pull(
            task_ids='create_hyperbrowser_profile', 
            key='profile_id'
        )
    
    # Get extension_id from XCom if not provided
    if not extension_id and context and 'task_instance' in context:
    ...
    if not profile_id:
        raise ValueError("No profile ID provided or found from previous task")
    ...
    try:
        ...
        # Store setup results in XCom if context available
        if context and 'task_instance' in context:
            context['task_instance'].xcom_push(key='setup_session_id', value=setup_session_id)
            context['task_instance'].xcom_push(key='setup_success', value=True)
            context['task_instance'].xcom_push(key='session_mongodb_id', value=session_mongodb_id)
...
def get_profile_by_id(profile_id, **context):
...
def get_profiles_by_type(profile_type='hyperbrowser_automa', **context):
...
def deactivate_profile(profile_id, reason='manual_deactivation', **context):
    """Deactivate a profile"""
    try:
        update_result = update_in_mongodb('chrome_profiles',
            {'profile_id': profile_id},
            {
                'is_active': False,
...
def create_workflow_session_from_profile(profile_id, workflow_type=None, **context):
    """Create a new session for workflow execution using an existing profile"""
    try:
        # Get the profile's extension
        profile = get_profile_by_id(profile_id, **context)
        if not profile:
...
# Direct usage without Airflow context:
